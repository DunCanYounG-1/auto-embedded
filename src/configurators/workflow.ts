...
import { unitFromRel, isEnabled } from "../content/packs";
 ...
 * `sel` 给定时按内容包过滤 tools/refs/modes（profile 感知装配）；不传 = 全装（向后兼容）。
 * scripts/ 与 workflow.md 是 RIPER 引擎/接线，永不过滤。
 */
export function getRuntimeManaged(sel?: Set<string>): Map<string, string> {
  ...
  for (const f of walkFiles(TPL.runtime)) {
    ...
    if (rel.startsWith("tools/") || rel.startsWith("refs/") || rel.startsWith("modes/")) {
      // tools/ = 工具 skill 的脚本/shared/references；refs/ = 嵌入式离线知识库；modes/ = 专项流程。
      // 三者均自包含、按需读取，原样装入（不做 python3 字面替换以免误改脚本逻辑或文档内容）。
      if (sel) {
        const u = unitFromRel(rel);
        if (u && !isEnabled(u, sel)) continue;
      }
