...
>
> 解决的痛点：你已经能用 `modes/matlab-embedded-toolkit.md` 算系数、能 `/build-cmake` 编译、能 `/flash-openocd` 烧录、能 `/serial-monitor` 抓日志，但**串起来需要 4 次"等待 + 复制 + 切窗口"**。本 mode 让 Claude 一次跑完 6 步并返回最终对比图。
...
- 已经知道 `modes/matlab-embedded-toolkit.md` 8 大场景的人；用本 mode 把单场景 + 后续步骤串起来
...
├──────────────────────────────────────────────────────────────────┤
│ Step 2: .mat → C 头文件                                            │
│   tool: tools/export_gains_to_c.py                                │
...
│   失败处置: 按 refs/failure-taxonomy.md 分类，不一定回 Step 1      │
├──────────────────────────────────────────────────────────────────┤
│ Step 4: 烧录                                                       │
...
## 2. 每步的 Command Outcome（按 `refs/contracts.md`）
...
--- Step 2: 导出 ---
bash: python tools/export_gains_to_c.py --input K.mat --mat-var K_disc
...
| `modes/matlab-embedded-toolkit.md` | 只覆盖 MATLAB 端（1 步） |
...
完整失败分类见 `refs/failure-taxonomy.md`。
