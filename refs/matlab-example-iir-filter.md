...
>
> 涉及 `modes/matlab-embedded-toolkit.md` 场景 2（滤波器设计）+ 场景 8（定点化）。
...
**判断噪声频率**：先用 `refs/matlab-example-serial-plot.md` + FFT（场景 3）看你的脏信号频谱。
...
#include "lpf_sos.h"     /* 由 export_gains_to_c.py 生成 */
#include "arm_math.h"
...
- 不知道 fc 选多少 → 先做 FFT（`modes/matlab-embedded-toolkit.md` 场景 3）
- 滤波后接 PID → `refs/matlab-example-step-id.md` §5
