...
>
> 涉及 `modes/matlab-embedded-toolkit.md` 场景 1（系统辨识）。
...
### 5.1 自动 PID 整定（接 `modes/matlab-embedded-toolkit.md` 场景 4）
...
1. → `refs/matlab-example-iir-filter.md`（场景 2 入门）：给温度信号加个一阶低通去除测量噪声
2. → 用 §5.1 的 PID 在 MCU 上跑闭环
3. → `refs/lqr-example-segway.md`（场景 4 进阶）：从单输入单输出升级到多状态 LQR
