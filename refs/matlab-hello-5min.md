...
>
> 适用：第一次接触 `embedded-dev` skill 的 MATLAB 工作流的人；不知道 `modes/matlab-embedded-toolkit.md` 8 大场景从哪开始的人。
...
如果 (1)(2) 中任意一项不对，**先别看本文**，按 `modes/mcp-healthcheck.md` 跑工具诊断。
...
❌ **失败**：参考 `modes/mcp-healthcheck.md` 排查。
...
- → `refs/matlab-example-step-id.md` — 步骤 2-3 进阶（不是看频谱而是辨识系统）
- → `refs/matlab-example-iir-filter.md` — 步骤 4 进阶（高阶滤波器、SOS、Q15 定点）
- → `refs/matlab-example-serial-plot.md` — 步骤 2 进阶（控制回路日志可视化、超调/调节时间）
- → `modes/matlab-embedded-toolkit.md` 决策树 — 你还有 8 个场景能用

如果想"一键完成 MATLAB → 编译 → 烧录 → 实测对比"全流程：

- → `modes/matlab-firmware-pipeline.md`（流水线 mode）
...
我想真的把这个滤波器装到我 STM32 上
   → refs/matlab-example-iir-filter.md §3.2/§3.3
   → modes/matlab-firmware-pipeline.md（一键流水线）

我想用 MATLAB 替代我现有的 Python LQR 工具
   → refs/lqr-example-bicycle-cornell.md

我想了解每个场景能干嘛
   → modes/matlab-embedded-toolkit.md（主 mode 决策树）

我想用 Simulink 建模
   → modes/matlab-embedded-toolkit.md 场景 9

我担心仿真对了上板不对
   → modes/matlab-embedded-toolkit.md 场景 10（MIL/SIL/PIL）
```
