...
>
> 对应 mode：`modes/industrial-data-acquisition.md`（IDA 主流程）+ `refs/cli-command-framework.md`（CLI 框架）
>
> 比赛模式：用 `modes/competition.md` v2，**跳过 CP-1.5 仿真阶段**（无算法需求），派 4 个 Agent（DRV/ALG/QA/REPORT）
...
| CLI 框架 | letter-shell / 自写 | **自写**（`refs/cli-command-framework.md`）| 控制 12 命令 + 多词 + 状态机 |
...
[QA]：
  ...
  - 验证 4 文件夹 + 文件命名规则
  - 验证 7 个评测流程 step
  - arch-check.sh + include-graph.py
...
参考 `refs/cli-command-framework.md`，注册 12 条命令。
...
参考 `modes/industrial-data-acquisition.md` §6。关键点：
...
| 任务 | 用哪个 skill / mode |
|---|---|
| GD32 HAL 工程 | `modes/gd32-board.md` + `refs/gd32f4xx-api.md` |
| 多 Agent 协同 | `modes/competition.md` v2 |
| sysFunction/ 集成 | `modes/industrial-data-acquisition.md` |
| 12 命令解析 | `refs/cli-command-framework.md` |
| 工程画像探测 | `python ~/.claude/skills/shared/project_detect.py` |
| 编译 | `/build-keil` 或 `/build-cmake` |
| 烧录 | `/flash-openocd` 或 `/flash-keil` |
| 串口测试 | `/serial-monitor` |
| 静态检查 | `scripts/arch-check.sh` + `tools/include-graph.py` |
| 答辩演练 | `modes/competition.md` v2 CP-5 |
...
- **集成框架**：`modes/industrial-data-acquisition.md`
- **CLI 框架**：`refs/cli-command-framework.md`
- **比赛模式 v2**：`modes/competition.md`
- **GD32 主板模板**：`modes/gd32-board.md`、`refs/gd32f4xx-api.md`
- **驱动移植**：`refs/driver-porting.md`
- **静态检查管线**：`refs/static-analysis-pipeline.md`、`scripts/arch-check.sh`
- **开源库索引**：`refs/embed-libs-index.md`（FatFs / SFUD / ebtn / ringbuffer 全部在内）
