...
所有 hook command 都走 polyglot 入口 `hooks/run-hook.cmd`：

```
"${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/embedded-dev}/hooks/run-hook.cmd" <script-name>
```
...
| 脚本扩展名 | 调用 | 用途 |
|---|---|---|
| `.py` | python / python3（先 python 避开 Windows Store stub） | UTF-8 强制 + Windows 路径规范化（如 `/c/Users/` → `C:\Users\`） |
| 无扩展名 | bash（Linux/macOS 原生 / Windows 用 Git Bash） | 简单 POSIX 工具组合 |
...
2. **frontmatter 注册命令** 通过 `run-hook.cmd <script>` 调用，不要直接调脚本（破坏 polyglot 跨平台兜底）
