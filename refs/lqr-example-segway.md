...
> 完整演示 `modes/matlab-embedded-toolkit.md` 场景 4（控制器设计）的"建模 → MATLAB 算 K → `.h` 导出 → C 闭环代码"全链路。
>
...
%% 9. 保存增益（供 export_gains_to_c.py 用）
...
- `main.c` 仅 4 句编排 ✓
- 跑 `bash scripts/arch-check.sh` exit 0 ✓
- 跑 `python tools/include-graph.py` 无 LAYER-VIOL ✓
...
详细系统调试方法见 `refs/systematic-debugging.md`。
