...
```bash
# bash（在工程根目录执行）
scripts/arch-check.sh             # ARCH-1~7 + ARCH-8
scripts/arch-check.sh --hw-check  # 仅 ARCH-8（硬件资源表 hw_lock 冲突）
scripts/arch-check.sh --no-hw     # 仅 ARCH-1~7
```

```powershell
# PowerShell（在工程根目录执行）
pwsh -File scripts/arch-check.ps1
pwsh -File scripts/arch-check.ps1 --hw-check
pwsh -File scripts/arch-check.ps1 --no-hw
```
...
- REVIEW Step 3「代码质量」要求**必跑**本门禁（依据 `refs/embedded-architecture.md` §7 依赖方向 + §8 屎山预警）。
