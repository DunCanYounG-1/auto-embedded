...
>
> 涉及 `modes/matlab-embedded-toolkit.md` 场景 7（数据可视化与日志分析）。
...
发现噪声 → 接 `refs/matlab-example-iir-filter.md` 设计低通滤波器。
...
- **后续动作**：
  1. 触发 `refs/matlab-example-iir-filter.md` 设计 50 Hz 陷波器
  2. 重整 PID（场景 4）
```
...
1. → `refs/matlab-example-step-id.md`（场景 1）：把测出来的响应做系统辨识
2. → `refs/matlab-example-iir-filter.md`（场景 2）：消除发现的噪声
