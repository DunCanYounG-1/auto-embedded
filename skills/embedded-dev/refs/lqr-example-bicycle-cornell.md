...
%% 7. 保存为 .mat 供 export_gains_to_c.py 链路用
...
方案 B：python tools\export_gains_to_c.py --input bicycle_lqr_table.mat
...
**路 B（要改 export_gains_to_c.py）**：加一个 `--format struct-array` 选项，输出 `LQR_GainEntry_t` 风格。如果你要走这条，我可以下一轮加这个 feature。
...
每条都是独立的子项目，本示例不展开；触发 `MPC` / `卡尔曼` / `增益调度` 关键词时 embedded-dev 会路由到 `modes/matlab-embedded-toolkit.md` 的对应场景。
