...
### 4.1 控制阈值 / 死区 / 限幅 / 增益的标定溯源（Required）

§4 的"禁止裸字面量"对**控制类常数**额外加严：PID 增益、死区、限幅、到达半径、采样周期 `dt` 等，不仅要命名常量，还必须在注释里带**四要素**——否则它就是不可移植的魔法数字。

| 要素 | 含义 | 反例 → 正例 |
|---|---|---|
| **单位** | SI 单位后缀 | `dz` → `YAW_HOLD_DEADBAND_DEG` |
| **来源** | `measured`(实测) / `derived`(派生) / `datasheet` / `tuned`(整定) | 注释标 `// tuned @ 12V, 见整定记录` |
| **测量方法** | 怎么得到的，可复现 | `// 电机死区辨识：扫 PWM 找起转点` |
| **适用硬件/条件** | 标定时的板子/电压/负载 | `// 仅 F407+GM37 12V，换硬件须重标` |

```c
/* ❌ 魔法数字：换板子/电压/电机全部失效，且无人知道怎么来的 */
if (pwm < 60) coast();
if (fabsf(gz_dps) > 3.0f) integrate_yaw();

/* ✅ 命名 + 四要素溯源 */
#define MOTOR_PWM_DEADZONE   60     /* tuned：PWM 死区辨识起转点；@TB6612+GM37 12V；换驱动须重标 */
#define GYRO_YAW_DZ_DPS      3.0f   /* measured：静止陀螺噪声 3σ；⚠副作用：吞 <3°/s 真实低速旋转，仅作零偏标定后的兜底 */
```

> 控制阈值/死区/增益与惯量、摩擦、电池电压、减速比、控制周期强耦合，**严禁跨工程照抄具体数值**。死区类常数必须额外标注副作用（如"吞低速信号"）。批量反例见 `.auto-embedded/refs/case-dcar-control-defects.md`；正面方案见 `.auto-embedded/refs/omni-wheel-odometry.md`。
