---
name: aemb-flash-jlink
description: 当需要使用 SEGGER J-Link 探针烧录固件，或启动 RTT 日志捕获时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/flash-jlink/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/flash-jlink/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/flash-jlink/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/flash-jlink/references/usage.md)，确认本次是环境探测、配置扫描，还是执行烧录。
2. 若不确定 J-Link 环境状态，先运行自带脚本 [scripts/jlink_flasher.py](.auto-embedded/tools/flash-jlink/scripts/jlink_flasher.py) 的 `--detect` 模式确认。
...
- 烧录成功后推荐 `aemb-serial-monitor` 或 `aemb-debug-jlink`。

## 交接关系

- 当下一步要看运行日志时，将成功烧录结果交给 `aemb-serial-monitor`。
- 当用户需要 GDB 调试时，将结果交给 `aemb-debug-jlink`。
