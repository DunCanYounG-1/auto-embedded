---
name: aemb-flash-keil
description: 当需要通过 Keil MDK 内置调试器烧录固件到目标板时使用，利用工程中已配置的调试器和 Flash 算法执行下载。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/flash-keil/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/flash-keil/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/flash-keil/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/flash-keil/references/usage.md)，确认本次操作。
2. 对于常见场景，直接一次调用完成探测+烧录：
   ```bash
   {{PYTHON_CMD}} .auto-embedded/tools/flash-keil/scripts/keil_flasher.py --detect --flash --project <工程文件> --target <目标名>
   ```
