---
name: aemb-debug-gdb-openocd
description: 当需要通过 OpenOCD 启动或附着 GDB 会话，调用自带脚本完成固件下载、在线调试或崩溃现场检查时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/debug-gdb-openocd/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/debug-gdb-openocd/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/debug-gdb-openocd/references/usage.md`。
...
- 探针由脚本自动探测，优先级与 `aemb-flash-openocd` 保持一致。
- 做符号级调试必须有 `ELF`。若只有 `HEX` 或 `BIN`，应阻塞并要求提供匹配 `ELF`。

## 执行步骤

1. 先阅读 [references/debug-playbook.md](.auto-embedded/tools/debug-gdb-openocd/references/debug-playbook.md) 中的决策树，判断应使用条件断点、普通断点还是 OpenOCD Telnet。
2. 再阅读 [references/usage.md](.auto-embedded/tools/debug-gdb-openocd/references/usage.md)，确认本次是环境探测，还是执行调试会话。
3. 若不确定环境是否就绪，先运行自带脚本 [scripts/gdb_debugger.py](.auto-embedded/tools/debug-gdb-openocd/scripts/gdb_debugger.py) 的 `--detect` 模式确认。
...
- 当复位后或继续运行后下一步是观察运行行为时，推荐 `aemb-serial-monitor`。

## 交接关系

- 当目标恢复运行后，需要继续观察运行期日志时，将成功会话交给 `aemb-serial-monitor`。