---
name: aemb-memory-analysis
description: 当需要解析 .map 文件或 ELF 获取固件内存使用报告、符号大小排名或版本对比时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/memory-analysis/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/memory-analysis/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/memory-analysis/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/memory-analysis/references/usage.md)，确认本次是环境探测、构建目录扫描，还是执行分析。
2. 若不确定工具是否可用，先运行 [scripts/memory_analyzer.py](.auto-embedded/tools/memory-analysis/scripts/memory_analyzer.py) 的 `--detect` 模式确认。
