...
> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/flash-idf/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/flash-idf/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/flash-idf/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/flash-idf/references/usage.md)，确认本次是环境探测、烧录、擦除，还是启动调试。
2. 若不确定环境状态，先运行自带脚本 [scripts/idf_flasher.py](.auto-embedded/tools/flash-idf/scripts/idf_flasher.py) 的 `--detect` 模式确认。
