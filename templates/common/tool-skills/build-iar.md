...
> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/build-iar/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/build-iar/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/build-iar/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/build-iar/references/usage.md)，确认本次是环境探测、工程扫描、列出配置，还是执行编译。
2. 若不确定环境是否就绪，先运行自带脚本 [scripts/iar_builder.py](.auto-embedded/tools/build-iar/scripts/iar_builder.py) 的 `--detect` 模式确认。
