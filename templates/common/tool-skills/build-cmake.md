---
name: aemb-build-cmake
description: 当需要配置或构建基于 CMake 的嵌入式固件工程，调用自带脚本执行构建并定位固件产物时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/build-cmake/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/build-cmake/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/build-cmake/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/build-cmake/references/usage.md)，确认本次是环境探测、列出预设、执行构建，还是仅扫描产物。
2. 若不确定环境是否就绪，先运行自带脚本 [scripts/cmake_builder.py](.auto-embedded/tools/build-cmake/scripts/cmake_builder.py) 的 `--detect` 模式确认。
