---
name: aemb-debug-jlink
description: 当需要通过 J-Link GDB Server 启动或附着 GDB 会话，完成固件下载、在线调试或崩溃现场检查时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/debug-jlink/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/debug-jlink/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/debug-jlink/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/debug-jlink/references/usage.md)，确认本次是环境探测，还是执行调试会话。
2. 若不确定环境是否就绪，先运行自带脚本 [scripts/jlink_debugger.py](.auto-embedded/tools/debug-jlink/scripts/jlink_debugger.py) 的 `--detect` 模式确认。
...
- 当用户需要 RTOS 线程感知调试时，将会话交给 `aemb-rtos-debug`。
