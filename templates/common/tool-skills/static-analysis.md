---
name: aemb-static-analysis
description: 当需要对嵌入式 C/C++ 代码运行 cppcheck、clang-tidy 或 GCC analyzer 静态分析，或进行 MISRA-C 合规检查时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/static-analysis/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/static-analysis/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/static-analysis/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/static-analysis/references/usage.md)，确认本次使用哪个分析工具。
2. 若不确定工具是否可用，先运行 [scripts/static_analyzer.py](.auto-embedded/tools/static-analysis/scripts/static_analyzer.py) 的 `--detect` 模式。
...
7. 阅读 [references/misra-quick-ref.md](.auto-embedded/tools/static-analysis/references/misra-quick-ref.md) 了解常见 MISRA 违规。
