---
name: aemb-debug-platformio
description: 当需要通过 PlatformIO 内置调试功能对目标板进行 GDB 调试时使用，支持下载暂停、附加和崩溃现场分析。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/debug-platformio/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/debug-platformio/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/debug-platformio/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/debug-platformio/references/usage.md)，确认本次操作。
2. 探测调试环境：
   ```bash
   {{PYTHON_CMD}} .auto-embedded/tools/debug-platformio/scripts/pio_debugger.py --detect --project-dir <工程目录> --env <环境名>
   ```
3. 执行调试：
   ```bash
   {{PYTHON_CMD}} .auto-embedded/tools/debug-platformio/scripts/pio_debugger.py --project-dir <工程目录> --env <环境名> --mode download-and-halt
   ```
