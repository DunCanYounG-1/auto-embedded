---
name: aemb-rtos-debug
description: 当需要进行 FreeRTOS/RT-Thread/Zephyr 的线程感知调试，查看任务列表、栈水位或死锁检测时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/rtos-debug/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/rtos-debug/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/rtos-debug/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/rtos-debug/references/usage.md) 和 [references/rtos-patterns.md](.auto-embedded/tools/rtos-debug/references/rtos-patterns.md)，了解常见 RTOS 问题模式。
2. 确保 GDB Server（OpenOCD 或 JLinkGDBServer）已在运行。
3. 若不确定 RTOS 类型，先运行 [scripts/rtos_debugger.py](.auto-embedded/tools/rtos-debug/scripts/rtos_debugger.py) 的 `--detect` 模式。
...
- GDB Server 需要由用户或上游 skill（`aemb-debug-gdb-openocd` 或 `aemb-debug-jlink`）预先启动。
...
- 当确认栈溢出时，建议使用 `aemb-memory-analysis` 分析固件内存布局。
- 当需要更深入的单步调试时，交给 `aemb-debug-gdb-openocd` 或 `aemb-debug-jlink`。
