---
name: aemb-serial-monitor
description: 当需要识别正确串口、调用自带脚本抓取日志，并分析嵌入式固件运行状态时使用。
---

> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/serial-monitor/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/serial-monitor/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/serial-monitor/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/serial-monitor/references/usage.md)，确认本次是列表、抓取、等待字符串、持续监视，还是“先监听后复位”。
2. 运行自带脚本 [scripts/serial_monitor.py](.auto-embedded/tools/serial-monitor/scripts/serial_monitor.py)。列出串口时使用 `--list`，自动选择端口时使用 `--auto`。
...
- 当日志表明程序崩溃、卡死或启动异常时，推荐 `aemb-debug-gdb-openocd`。

## 交接关系

- 当日志显示需要断点、寄存器或回溯时，将结果交给 `aemb-debug-gdb-openocd`。
