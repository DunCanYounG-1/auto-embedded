...
> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/build-idf/`，用 `{{PYTHON_CMD}}` 运行 `.auto-embedded/tools/build-idf/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/build-idf/references/usage.md`。
...
1. 先阅读 [references/usage.md](.auto-embedded/tools/build-idf/references/usage.md)，确认本次是环境探测、设置目标、执行构建，还是仅扫描产物。
2. 若不确定环境是否就绪，先运行自带脚本 [scripts/idf_builder.py](.auto-embedded/tools/build-idf/scripts/idf_builder.py) 的 `--detect` 模式确认。
...
- 成功后推荐 `aemb-flash-idf`。

## 交接关系

- 当下一步意图是给硬件烧录程序时，将成功构建结果交给 `aemb-flash-idf`。
